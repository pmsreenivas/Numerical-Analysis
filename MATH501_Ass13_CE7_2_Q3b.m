...
x4 = zeros(1,m);
x0(1) = iv;
x1(1) = (1)/(x0(1))^2 - x0(1)*1;
x2(1) = (-2)/(x0(1))^3 - x1(1)*1 - x0(1);
x3(1) = (6)/(x0(1))^4 - x2(1)*1 - 2*x1(1);
x4(1) = (-24)/(x0(1))^5 - x3(1)*1 - 3*x2(1);
for i = 2:m
    pt = 1 + (i - 1)*h;
    x0(i) = x0(i-1) + h*(x1(i-1)) + ((h^2)*x2(i-1))/2 + ((h^3)*(x3(i-1)))/6 + ((h^4)*(x4(i-1)))/24;
    cv = x0(i);
    x1(i) = (1)/(x0(i))^2 - x0(i)*pt;
    x2(i) = (-2)/(x0(i))^3 - x1(i)*pt - x0(i);
    x3(i) = (6)/(x0(i))^4 - x2(i)*pt - 2*x1(i);
    x4(i) = (-24)/(x0(i))^5 - x3(i)*pt - 3*x2(i);
end
TS = cv;
