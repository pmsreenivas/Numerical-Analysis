...
s0 = str2sym(S0);
s = str2sym(S);
xval = input("Enter the value of x at which the derivative is to be computed:- ");
if(xval > t0 && xval < t(1))
    g = s0;
else
    for i = 1:n-1
       if(xval > t(i) && xval < t(i + 1))
           g = s(i);
           break;
       end
    end
end
D = eval(subs(diff(g),xval));
D = double(D);
disp("The derivative of the spline at " + num2str(xval) + " is " + num2str(D));